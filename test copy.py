...
class TestImage(Lab1Test):
    def test_load_color(self):
        result = Filter_and_Seam_Carving.load_color_image('test_images/centered_pixel.png')
        expected = {
            'height': 11,
            'width': 11,
...
class TestColorFilters(Lab1Test):
    def test_color_filter_inverted(self):
        im = Filter_and_Seam_Carving.load_color_image('test_images/centered_pixel.png')
        color_inverted = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.inverted)
    ...
    def test_color_filter_edges(self):
        im = Filter_and_Seam_Carving.load_color_image('test_images/centered_pixel.png')
        color_edges = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.edges)
    ...
    def test_color_filters(self):
        for fname in ('frog', 'tree'):
            for filt, filt_name in {(Filter_and_Seam_Carving.edges, 'edges'), (Filter_and_Seam_Carving.inverted, 'inverted')}:
                with self.subTest(f=fname):
                    ...
                    im = Filter_and_Seam_Carving.load_color_image(inpfile)
                    oim = object_hash(im)
                    color_filter = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(filt)
                    self.assertTrue(callable(color_filter), 'color_filter_from_greyscale_filter should return a function.')
                    result = color_filter(im)
                    expected = Filter_and_Seam_Carving.load_color_image(expfile)
                    self.assertEqual(object_hash(im), oim, 'Be careful not to modify the original image!')
                    self.compare_color_images(result, expected)

    def test_blur_filter_1(self):
        blur_filter = Filter_and_Seam_Carving.make_blur_filter(3)
        self.assertTrue(callable(blur_filter), 'make_blur_filter should return a function.')
        color_blur = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(blur_filter)
        im = Filter_and_Seam_Carving.load_color_image('test_images/centered_pixel.png')
        ...
        for fname in ('cat', 'mushroom'):
            for ker_size in {3, 5}:
                with self.subTest(f=fname):
                    ...
                    im = Filter_and_Seam_Carving.load_color_image(inpfile)
                    oim = object_hash(im)
                    blur_filter = Filter_and_Seam_Carving.make_blur_filter(ker_size)
                    self.assertTrue(callable(blur_filter), 'make_blur_filter should return a function.')
                    color_blur = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(blur_filter)
                    result = color_blur(im)
                    expected = Filter_and_Seam_Carving.load_color_image(expfile)
        ...
        for fname in ('construct', 'bluegill'):
            for ker_size in {3, 5}:
                with self.subTest(f=fname):
                    ...
                    im = Filter_and_Seam_Carving.load_color_image(inpfile)
                    oim = object_hash(im)
                    sharpened_filter = Filter_and_Seam_Carving.make_sharpen_filter(ker_size)
                    self.assertTrue(callable(sharpened_filter), 'make_sharpen_filter should return a function.')
                    color_sharpen = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(sharpened_filter)
...
class TestCascade(Lab1Test):
    def setUp(self):
        self.color_edges = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.edges)
        self.color_inverted = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.inverted)
        self.color_blur_5 = Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.make_blur_filter(5))
    ...
    def test_cascade_1(self):
        ...
        f_cascade = Filter_and_Seam_Carving.filter_cascade([f1, f2, f3])
    ...
    def test_cascades(self):
        cascade0 = [self.color_edges,
                    Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.make_sharpen_filter(3))]
        cascade1 = [Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.make_blur_filter(5)),
                    self.color_edges,
                    Filter_and_Seam_Carving.color_filter_from_greyscale_filter(Filter_and_Seam_Carving.make_sharpen_filter(3))]
        ...
        for fname in ('tree',):
            for cix, cascade in enumerate([cascade0, cascade1]):
                ...
                im = Filter_and_Seam_Carving.load_color_image(inpfile)
                oim = object_hash(im)
                f_cascade = Filter_and_Seam_Carving.filter_cascade(cascade)
                self.assertTrue(callable(f_cascade), 'filter_cascade should return a function.')
                result = f_cascade(im)
                expected = Filter_and_Seam_Carving.load_color_image(expfile)
                self.assertEqual(object_hash(im), oim, 'Be careful not to modify the original image!')
...
class TestSeamCarvingHelpers(Lab1Test):
    def test_greyscale(self):
        for fname in ('pattern', 'smallfrog', 'bluegill', 'twocats', 'tree'):
            ...
            grey = Filter_and_Seam_Carving.greyscale_image_from_color_image(im)
    ...
    def test_energy(self):
        for fname in ('pattern', 'smallfrog', 'bluegill', 'twocats', 'tree'):
            ...
            oim = object_hash(im)
            result = Filter_and_Seam_Carving.compute_energy(im)
    ...
    def test_cumulative_energy(self):
        for fname in ('pattern', 'smallfrog', 'bluegill', 'twocats', 'tree'):
            ...
            with open(infile, 'rb') as f:
                energy = pickle.load(f)
            result = Filter_and_Seam_Carving.cumulative_energy_map(energy)
    ...
    def test_min_seam_indices(self):
        for fname in ('pattern', 'smallfrog', 'bluegill', 'twocats', 'tree'):
            ...
            with open(infile, 'rb') as f:
                cem = pickle.load(f)
            result = Filter_and_Seam_Carving.minimum_energy_seam(cem)
    ...
    def test_seam_removal(self):
        for fname in ('pattern', 'bluegill', 'twocats', 'tree'):
            ...
            result = Filter_and_Seam_Carving.image_without_seam(Filter_and_Seam_Carving.load_color_image(imfile), seam)

            expfile = os.path.join(TEST_DIRECTORY, 'test_results', f'{fname}_1seam.png')

            self.compare_color_images(result, Filter_and_Seam_Carving.load_color_image(expfile))
...
class TestSeamCarving(Lab1Test):
    def test_endtoend_centeredpixel(self):
        ...
        for i in range(1, 11):
            result = Filter_and_Seam_Carving.seam_carving(im, i)
            self.assertEqual(object_hash(im), oim, 'Be careful not to modify the original image!')
